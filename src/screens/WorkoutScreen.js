...
export default function WorkoutScreen({ navigation }) {
  ...
  // Render a workout day full-screen (NOT in a Modal) so swipe-back works —
  // for both custom-built workouts and days from the app-generated plan.
  if (viewWorkout) {
    return <WorkoutDayScreen day={viewWorkout} onClose={() => { setViewWorkout(null); load(); }} />;
  }
  if (selectedDay) {
    return <WorkoutDayScreen day={selectedDay} onClose={() => { setSelectedDay(null); load(); }} />;
  }
