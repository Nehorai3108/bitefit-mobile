  ...
  const stopRec = async () => {
    ...
    try {
      ...
      if (uri) {
        ...
        if (text) send(text);              // clean transcription → send automatically
...
const makeStyles = (C) => StyleSheet.create({
  ...
  onlineDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#3a7a4a' },
  ...
  loggedTxt: { color: '#3a7a4a', fontSize: 13, fontWeight: '700', marginTop: 8, textAlign: 'center' },
  ...
  recipeMeta: { color: '#3a7a4a', fontSize: 12, fontWeight: '700', textAlign: 'right', marginTop: 2 },
